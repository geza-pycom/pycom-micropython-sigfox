...
STATIC void gatts_char_callback_handler(void *arg) {
    ...
    if (chr->handler && chr->handler != mp_const_none) {
        ...
        if(((gatts_char_cbk_arg_t*)arg)->data_length > 0) {
            ...
            heap_caps_free(((gatts_char_cbk_arg_t*)arg)->data);
        }
    ...
    heap_caps_free((gatts_char_cbk_arg_t*)arg);
}
        ...
        if (attr_obj) {
            if (attr_obj->is_char) {
                ...
                if (char_obj->trigger & MOD_BT_GATTS_READ_EVT) {
                    ...
                    gatts_char_cbk_arg_t *cbk_arg = heap_caps_malloc(sizeof(gatts_char_cbk_arg_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
